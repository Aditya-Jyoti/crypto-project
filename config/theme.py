...
def make_frame(parent, **kwargs) -> tk.Frame:
    return tk.Frame(parent, bg=kwargs.pop("bg", COLORS["bg_panel"]), **kwargs)


def make_label(
    parent, text: str, style: str = "body", color: str = "text_primary", **kwargs
) -> tk.Label:
    return tk.Label(
        parent,
        text=text,
...
def make_button(
    parent, text: str, command=None, color: str = "accent_blue", **kwargs
) -> tk.Button:
    fg_col = COLORS[color]
    btn = tk.Button(
        parent,
...
def role_header(
    parent, title: str, subtitle: str, accent: str = "accent_blue"
) -> tk.Frame:
    ...
    tk.Label(
        inner, text=title, font=FONTS["title"], bg=COLORS["bg_dark"], fg=COLORS[accent]
    ).pack(anchor="w")
    tk.Label(
        inner,
        text=subtitle,
